...
int countWays(string input,bool result)
{
  ...
  if(input.length()==1)
  {
    ...
    //return evalBool(input[0])==true?1:0;
    cout<<"bool eval :"<<(input.compare("0")==0)<<endl;
    return (input.compare("0")==0) ? 0:1;
  }
  int ways=0;
  for(int i=1;i<input.length();i+=2)
  {
    ...
    int t_w = result  ? totalTrue : (total - totalTrue);    
    ways+=t_w;
  }
  cout<<"ways :"<<ways<<endl;
  return ways;
...
int main()
{
  //string operations = "l^0|0|1"; //l^0|0|1
  //cout<<countWays(operations,false);
  cout<<evalBool('1');
  return 0;
}
