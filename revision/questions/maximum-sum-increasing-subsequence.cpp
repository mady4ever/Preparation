/*
  ...
  varient of following problem so reusing same code.
...
int longestIncreasingSumSequence(int a[],int s)
{
  int buf[s];
  for(int i=0;i<s;i++)
     buf[i] = a[i];
  for(int i=1;i<s;i++)
  {
    for(int j=i-1;j>=0;j--)
    {
      if(a[i] > a[j] && (buf[j]+a[i]) > buf[i])
      {
         buf[i] = max(buf[i],(buf[j]+a[i]));
      }
    }
  }
  return *max_element(buf,buf+s);
...
{ 
  ...
  cout<<endl;
  cout<<longestIncreasingSumSequence(arr,n); //10 + 22 + 33 + 50 + 60
  return 0;
}
