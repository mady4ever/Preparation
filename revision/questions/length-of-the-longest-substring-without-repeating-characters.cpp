/*
  ...
  create one buffer array which holds the index of last visited char.
  initialize the buffer array with -1
  if current char is not visited then increase the current length 
  if last char is encountered then change update the current length.
*/
